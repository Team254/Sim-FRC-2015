...
public class ServoMechanism {
     ...
     * 
     * @param input
     *            The source of the PWMs controlling the mechanism.
     * @param output
     *            The position and rate of the mechanism will be written to this
     *            object.
     * @param pdp_channel
     *            The channel # of the PDP slot for the motor.
     * @param encoder_distance_per_tick
     *            How to convert between encoder units and real world distance.
     * @param model
     *            The DCMotor representing the system dynamics.
     * @param load
     *            The applied load to the mechanism, in kg*m^2
     * @param limits
     *            Minimum and maximum position limits for the mechanism.
     */
    public ServoMechanism(PWMObserver input, EncoderSetter output,
     ...
     * 
     * @param load
     *            The current mechanism load (in kg*m^2)
     */
     ...
     * 
     * @param position
     *            The new position of the mechanism.
     */
     ...
     *            The current battery voltage (V).
     * @param acceleration
     *            The external acceleration being experienced by the mechanism
     *            at the motor (ex. due to gravity) (rad/s^2).
     * @param timestep
     *            The simulation period (s).
     */
    public void step(double battery_voltage, double acceleration,
            double timestep) {
        double command = m_input.get() * battery_voltage;
        m_model.step(command, m_load, acceleration, timestep);
        m_output.set((int) ((m_model.getPosition() - m_start_position) / m_encoder_distance_per_tick));
     ...
     * 
     * @return True if the current position is greater than or equal to the
     *         minimum.
     */
    public boolean withinLowerLimits() {
        return m_model.getPosition() >= (m_limits.min_position - m_start_position);
    }

    /**
     * Check the upper position limit.
     * 
     * @return True if the current position is less than or equal to the
     *         maximum.
     */
    public boolean withinUpperLimits() {
        return m_model.getPosition() <= (m_limits.max_position - m_start_position);
    }
}
